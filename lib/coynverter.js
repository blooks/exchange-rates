 ...
 * @return {[type]}                 [description]
 */
var getExchangeRateForOneDate = function (date, currency, collectionToWriteName, collection, callback) {
  /**
 ...
 * @return {[type]}            [description]
 */
var getExchangeRatesForNewCurrency = function (currency, collectionToWrite, callback) {
  ...
  return request.get({uri: url}, function (err, response, body) {
    ...
    if(body){
      ...
      _connectToDatabase(function (err, database) {
        if(database){
          database.collection(collectionToWrite, function (error, collection) {
            if(collection){
...
(function() {
        ...
        database.collection(collectionToUpdate, function (error, collection) {
          if(collection){
            try {
              ...
              getExchangeRateForOneDate(todayUpdate, currency, collectionToUpdate, collection, function (err, result) {
                log.info(result);
                log.error(err);
   ...
   * @return {[type]}                    [description]
   */
  coynverter.convert = function (date, currency, collectionToRead, callback) {
    return db.open(function (err, db) {
      if(db){
